...
module lock_micro_housing() {
        cuboid([1.6, 4.2, 1.2], chamfer=.3, align=V_TOP, edges=EDGES_Z_ALL);
    translate([.1, 0, .3]) cuboid([1.8, 4.2, 1.4], chamfer=.3, align=V_TOP);
    // for better rendering
    translate([0, 0, .3]) cuboid([1.6, 4.2, 1], chamfer=.3, align=V_BOTTOM, edges=EDGES_Z_ALL);
}
...
module lock_micro_narrow_housing() {
    cuboid([1.6, 2.2, 1.2], chamfer=.3, align=V_TOP,, edges=EDGES_Z_ALL);
    translate([.1, 0, .3]) cuboid([1.8, 2.2, 1.4], chamfer=.3, align=V_TOP);
    // for better rendering
    translate([0, 0, .3]) cuboid([1.6, 2.2, 1], chamfer=.3, align=V_BOTTOM, edges=EDGES_Z_ALL);
}
